...
<div class="max-w-md mx-auto px-4 pt-6 pb-8">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-xl font-bold text-stone-800">Tambah Pohon Massal</h1>
            <p class="text-xs text-stone-400 font-mono">{{ $greenhouse->name }}</p>
        </div>
    </div>

    <form method="POST" action="{{ route('greenhouse.trees.bulk.store', $greenhouse) }}" class="bg-white rounded-2xl shadow-sm border border-stone-100 p-6 space-y-5">
        @csrf
        <div class="bg-amber-50 border border-amber-200 rounded-xl p-3 text-sm text-amber-700">
            <strong>Tips:</strong> Masukkan prefix dan jumlah. Nomor pohon akan dibuat otomatis, misal: <code class="bg-amber-100 px-1 rounded">GH001-0001</code> s/d <code class="bg-amber-100 px-1 rounded">GH001-0100</code>
        </div>
        <div>
            <label class="block text-sm font-medium text-stone-600 mb-1.5">Prefix Nomor Pohon <span class="text-red-500">*</span></label>
            <input type="text" name="prefix" value="{{ old('prefix', $greenhouse->code) }}" required class="w-full px-4 py-3 border border-stone-200 rounded-xl text-sm font-mono uppercase focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" placeholder="GH-001">
            @error('prefix') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>
        <div>
            <label class="block text-sm font-medium text-stone-600 mb-1.5">Jumlah Pohon <span class="text-red-500">*</span></label>
            <input type="number" name="count" value="{{ old('count', 100) }}" required min="1" max="2000" class="w-full px-4 py-3 border border-stone-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" placeholder="1 - 2000">
            @error('count') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @endError
        </div>
        <div>
            ...
            <select name="melon_variety_id" class="w-full px-4 py-3 border border-stone-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all bg-white">
                ...
                @foreach($varieties as $v)
                    <option value="{{ $v->id }}" {{ old('melon_variety_id') == $v->id ? 'selected' : '' }}>{{ $v->name }}</option>
                @endforeach
            </select>
        </div>
        <div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <label class="relative flex cursor-pointer">
                    <input type="radio" name="status" value="alive" {{ old('status', 'alive') === 'alive' ? 'checked' : '' }} class="peer sr-only">
                ...
                <label class="relative flex cursor-pointer">
                    <input type="radio" name="status" value="dead" {{ old('status') === 'dead' ? 'checked' : '' }} class="peer sr-only">
