...
<div class="max-w-md mx-auto px-4 pt-6 pb-8">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-xl font-bold text-stone-800">Tambah Pohon</h1>
            <p class="text-xs text-stone-400 font-mono">{{ $greenhouse->name }}</p>
        </div>
    </div>

    <form method="POST" action="{{ route('greenhouse.trees.store', $greenhouse) }}" class="bg-white rounded-2xl shadow-sm border border-stone-100 p-6 space-y-5">
        @csrf
        <div>
            <label class="block text-sm font-medium text-stone-600 mb-1.5">Nomor Pohon <span class="text-red-500">*</span></label>
            <input type="text" name="tree_number" value="{{ old('tree_number') }}" required class="w-full px-4 py-3 border border-stone-200 rounded-xl text-sm font-mono uppercase focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" placeholder="GH-001-0001">
        ...
        <div>
            <label class="block text-sm font-medium text-stone-600 mb-3">Status Pohon <span class="text-red-500">*</span></label>
