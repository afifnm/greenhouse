...
<div class="max-w-md mx-auto px-4 pt-6 pb-8">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-xl font-bold text-stone-800">Tambah Data Buah</h1>
            <p class="text-xs text-stone-400 font-mono">{{ $tree->tree_number }}</p>
        </div>
    </div>

    <form method="POST" action="{{ route('greenhouse.fruits.store', [$greenhouse, $tree]) }}" class="bg-white rounded-2xl shadow-sm border border-stone-100 p-6 space-y-5">
        @csrf
        <div>
            <label class="block text-sm font-medium text-stone-600 mb-3">Kondisi Buah <span class="text-red-500">*</span></label>
            <div class="grid grid-cols-2 gap-3">
                <label class="relative flex cursor-pointer">
                    <input type="radio" name="condition" value="good" {{ old('condition', 'good') === 'good' ? 'checked' : '' }} class="peer sr-only">
                ...
                <label class="relative flex cursor-pointer">
                    <input type="radio" name="condition" value="rotten" {{ old('condition') === 'rotten' ? 'checked' : '' }} class="peer sr-only">
        ...
        <div>
            ...
            <div class="grid grid-cols-4 gap-2">
                ...
                <label class="relative flex cursor-pointer">
                    <input type="radio" name="grade" value="{{ $grade }}" {{ old('grade', 'A') === $grade ? 'checked' : '' }} class="peer sr-only">
                    <div class="w-full py-3 rounded-xl border-2 border-stone-200 bg-white text-center peer-checked:border-amber-500 peer-checked:bg-amber-50 transition-all">
                        <span class="text-sm font-bold {{ $grade === 'A' ? 'text-amber-700' : 'text-stone-600' }}">{{ $grade }}</span>
                    </div>
                </label>
                @endforeach
            </div>
            <div class="mt-2 text-xs text-stone-400 space-y-1">
                <p><strong>A:</strong> Premium — <strong>B:</strong> Baik — <strong>C:</strong> Sedang — <strong>D:</strong> Kurang</p>
            </div>
        </div>
        ...
        <div>
            <label class="block text-sm font-medium text-stone-600 mb-1.5">Catatan <span class="text-stone-400">(opsional)</span></label>
            <textarea name="notes" rows="3" class="w-full px-4 py-3 border border-stone-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all resize-none" placeholder="Catatan tambahan...">{{ old('notes') }}</textarea>
        </div>
